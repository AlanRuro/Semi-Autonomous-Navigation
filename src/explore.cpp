...
namespace explore {
  Explore::Explore()
    ...
    // Save map
    this->get_parameter("map_file", map_file_name_);
    save_map_client_ = this->create_client<slam_toolbox::srv::SaveMap>("slam_toolbox/save_map");
    RCLCPP_INFO(logger_, "Waiting to connect to save_map slam_toolbox service");
    save_map_client_->wait_for_service();
    RCLCPP_INFO(logger_, "Connected to save_map slam_toolbox service");
  ...
  void Explore::makePlan() {
    ...
    for (size_t i = 0; i < frontiers.size(); ++i) {
      ...
      RCLCPP_INFO(logger_, "frontier %zd size: %d", i, frontiers[i].size);
      RCLCPP_INFO(logger_, "frontier %zd min distance: %f", i, frontiers[i].min_distance);
    }
    ...
    if ((this->now() - last_progress_ > tf2::durationFromSec(progress_timeout_)) && !resuming_) {
      ...
      RCLCPP_WARN(logger_, "Adding current goal to black list");
      makePlan();
      return;
    }
  ...
  bool Explore::goalOnBlacklist(const geometry_msgs::msg::Point &goal) {
    constexpr static size_t tolerace = 1; // 5
